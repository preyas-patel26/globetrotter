...
import { Compass, Mail, Lock, ShieldCheck, UserCheck } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  const handleLoginSubmit = (targetEmail?: string) => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      if (success) {
        if (loginEmail.trim().toLowerCase() === 'admin@globetrotter.com') {
          navigate('/admin');
        } else {
          navigate('/dashboard');
        }
  ...
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row bg-background">
      ...
      {/* Right 50% - Login Form Glass Card */}
      <div className="w-full md:w-1/2 flex items-center justify-center p-6 md:p-12 bg-gradient-to-br from-surface via-background to-surface-container-low">
        <div className="w-full max-w-md glass-card-elevated rounded-3xl p-8 md:p-10 shadow-2xl border border-outline-variant/30">
          <div className="mb-6">
            ...
            <p className="text-outline text-sm mt-1">Please select a module or enter details to sign in.</p>
          </div>
          ...
          {/* Quick Module Login Switcher Buttons */}
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button
              type="button"
              onClick={() => {
                setEmail('demo@globetrotter.com');
                handleLoginSubmit('demo@globetrotter.com');
              }}
              className="p-3 rounded-2xl border border-primary/30 bg-primary-container/20 hover:bg-primary-container/40 text-left transition-all group"
            >
              <div className="flex items-center gap-1.5 text-primary font-bold text-xs">
                <UserCheck className="w-4 h-4" />
                <span>Traveler Login</span>
              </div>
              <span className="text-[10px] text-outline mt-0.5 block truncate">
            ...
            <button
              ...
              onClick={() => {
                ...
                handleLoginSubmit('admin@globetrotter.com');
              }}
              className="p-3 rounded-2xl border border-purple-200 bg-purple-50 hover:bg-purple-100 text-left transition-all group"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  placeholder="enter email (e.g. demo@globetrotter.com)"
            ...
            <div className="flex items-center justify-between text-xs pt-1">
              ...
              <a href="#forgot" onClick={(e) => { e.preventDefault(); alert("Password reset instructions sent to your email!"); }} className="text-primary font-semibold hover:underline">
                Forgot password?
              </a>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3.5 rounded-xl bg-primary-fixed text-on-primary-fixed font-bold text-sm shadow-md hover:bg-primary-fixed-dim hover:shadow-lg transition-all active:scale-[0.99] flex items-center justify-center mt-2"
            >
              {isLoading ? (
